...
Now that we've written our model in terms of $\mu$ and $\sigma$, it becomes easier to see how a model could take AB into consideration. We simply define $\mu$ so that it includes $\log(\mbox{AB})$ as a linear term:
